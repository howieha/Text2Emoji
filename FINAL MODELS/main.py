...
from CNN_unigram import CNN_unigram
from CNN_bigram import CNN_bigram
from CNN_trigram import CNN_trigram
...
def Train(train_loader, val_loader, weight_pos):
    ...
    if sys.argv[1] == "FFNN":
        model = FFNN(NUM_TASKS, BATCH_SIZE, DIM_EMB).cuda()
    elif sys.argv[1] == "LSTM":
        model = LSTM(NUM_TASKS, BATCH_SIZE, DIM_EMB).cuda()
    elif sys.argv[1] == "LSTM_bi":
        model = LSTM_bi(NUM_TASKS, BATCH_SIZE, DIM_EMB).cuda()
    elif sys.argv[1] == "LSTM_attention":
        model = LSTM_attention(NUM_TASKS, BATCH_SIZE, DIM_EMB).cuda()
    elif sys.argv[1] == "CNN_unigram":
        model = CNN_unigram(NUM_TASKS, BATCH_SIZE, DIM_EMB).cuda()
    elif sys.argv[1] == "CNN_bigram":
        model = CNN_bigram(NUM_TASKS, BATCH_SIZE, DIM_EMB).cuda()
    elif sys.argv[1] == "CNN_trigram":
        model = CNN_trigram(NUM_TASKS, BATCH_SIZE, DIM_EMB).cuda()
    elif sys.argv[1] == "CNN_attention":
        model = CNN_attention(NUM_TASKS, BATCH_SIZE, DIM_EMB).cuda()
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        torch.save(weight_pos, 'weight_pos.pt')
        # weight_pos = torch.load('weight_pos.pt')
