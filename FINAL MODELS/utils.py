...
def load_data_nn(dataset, num_class):
    ...
    X_train = make_same_length(X_train, LEN_SEQ)
    X_test = make_same_length(X_test, LEN_SEQ)
    X_val = make_same_length(X_val, LEN_SEQ)
    ...
    weight_pos = (((len(Y_train) + len(Y_test) + len(Y_val)) - num_pos) / num_pos) * WEIGHT_COEF
